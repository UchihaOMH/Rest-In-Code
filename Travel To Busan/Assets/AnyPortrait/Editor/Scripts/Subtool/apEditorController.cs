	...
	public class apEditorController
	{
													...
													bool isRefresh = true)
		{
			...
			try
			{
				...
				if (isRoot)
				{
					...
					//안내 메시지를 보여주자 > 그냥 패스
					//apStringWrapper strAddedObjects = new apStringWrapper(256);
					//int nDuplicatedObjects = 0;
					//foreach (KeyValuePair<apMeshGroup, apMeshGroup> src2Dst_MeshGroup in convertInfo.Src2Dst_MeshGroup)
					//{
					//	strAddedObjects.Append(src2Dst_MeshGroup.Value._name, false);
					//	strAddedObjects.Append(apStringFactory.I.Return, false);
					//	nDuplicatedObjects++;
					//}

					//foreach (KeyValuePair<apAnimClip, apAnimClip> src2Dst_AnimClip in convertInfo.Src2Dst_AnimClip)
					//{
					//	strAddedObjects.Append(src2Dst_AnimClip.Value._name, false);
					//	strAddedObjects.Append(apStringFactory.I.Return, false);
					//	nDuplicatedObjects++;
					//}
					//if(nDuplicatedObjects <= 1)
					//{
					//	strAddedObjects.Append(nDuplicatedObjects.ToString(), false);
					//	strAddedObjects.Append(" object is Duplicated.", true);
					//}
					//else
					//{
					//	strAddedObjects.Append(nDuplicatedObjects.ToString(), false);
					//	strAddedObjects.Append(" objects are Duplicated.", true);
					//}
					

					//EditorUtility.DisplayDialog("Duplicate Result", strAddedObjects.ToString(), Editor.GetText(TEXT.Okay));
				}
		...
		public apTransform_Mesh DuplicateMeshTransformInSameMeshGroup(apTransform_Mesh srcMeshTransform)
		{
			...
			try
			{
				...
				//메시 그룹 선택
				//Editor.Select.SetMeshGroup(newMeshGroup);
				Editor.Select.MeshGroup.ResetRenderUnits();
				Editor.Select.MeshGroup.RefreshModifierLink();
				Editor.Select.MeshGroup.SortRenderUnits(true);

				Editor.Select.SetSubMeshInGroup(dstMeshTransform);
		...
		public apTransform_MeshGroup DuplicateMeshGroupTransformInSameMeshGroup(apTransform_MeshGroup srcMeshGroupTransform)
		{
			...
			try
			{
				...
				//Matrix를 동일하게 설정한다.
				dstMeshGroupTransform._matrix.SetMatrix(srcMeshGroupTransform._matrix);
				...
				Editor.Select.SetSubMeshGroupInGroup(dstMeshGroupTransform);
			...
			for (int iPS = 0; iPS < nParamSet; iPS++)
			{
				...
				if(isUseModBone)
				{
					...
					for (int iSrcModBone = 0; iSrcModBone < nModBone; iSrcModBone++)
					{
						...
						dstModBone._transformMatrix.SetMatrix(srcModBone._transformMatrix);
					}
				}
			}
		...
		public bool MigrateMeshTransformToOtherMeshGroup(apTransform_Mesh targetMeshTransform, apMeshGroup srcMeshGroup, apMeshGroup dstMeshGroup)
		{
			...
			try
			{
				...
				while (true)
				{
					...
					prevLocal2WorldMatrix.RMultiply(curSubParentMGTF._matrix);
				...
				while (true)
				{
					...
					nextLocal2WorldMatrix.RMultiply(curSubParentMGTF._matrix);
				...
				targetWorldMatrix.RMultiply(prevLocal2WorldMatrix);

				//World > NextLocal2World_Inv > NextLocal
				apMatrix nextLocalMatrix = new apMatrix(targetWorldMatrix);
				nextLocalMatrix.RInverse(nextLocal2WorldMatrix);

				//변환 결과를 저장하자
				targetMeshTransform._matrix.SetMatrix(nextLocalMatrix);
				...
				Editor.Select.SetSubMeshInGroup(targetMeshTransform);
														...
														)
		{
			...
			newMeshTransform._matrix.SetMatrix(srcMeshTransform._matrix);
																	...
																	Dictionary<apMeshGroup, apMeshGroup> srcMeshGroup2NewMeshGroup)
		{
			...
			//나머지는 값 그대로 복사
			newMeshGroupTransform._matrix.SetMatrix(srcMeshGroupTransform._matrix);
													...
													bool isRecordUndo, bool isRefresh)
		{
			...
			if (isRefresh)
			{
				Editor.Select.SetMeshGroup(dstMeshGroup);
				Editor._portrait.LinkAndRefreshInEditor(false, dstMeshGroup);
											...
											List<apBone> duplicatedBones)
		{
			...
			newBone._defaultMatrix.SetMatrix(srcBone._defaultMatrix);
				...
				for (int iSrcPSG = 0; iSrcPSG < nParamSetGroups; iSrcPSG++)
				{
					...
					for (int iSrcPS = 0; iSrcPS < nSrcParamSets; iSrcPS++)
					{
						...
						for (int iSrcModMesh = 0; iSrcModMesh < nSrcModMeshes; iSrcModMesh++)
						{
							...
							//나머지 정보를 복사하자
							dstModMesh._transformMatrix.SetMatrix(srcModMesh._transformMatrix);
		...
		public apBakeResult Bake()
		{
			...
			CheckAndChangeTextureDataColorSpace(Editor._portrait);
										...
										int windowHeight, int windowScrollY)
		{
			...
			EditorGUILayout.BeginHorizontal(apGUILOFactory.I.Width(width), apGUILOFactory.I.Height(presetIconSize));
			GUILayout.Box(Editor.ImageSet.Get(apEditorUtil.GetControlParamPresetIconType(controlParam._iconPreset)), GUI.skin.label, apGUILOFactory.I.Width(presetIconSize), apGUILOFactory.I.Height(presetIconSize));

			EditorGUILayout.LabelField(controlParam._keyName, apGUILOFactory.I.Width(labelWidth - (presetIconSize + 4)), apGUILOFactory.I.Height(presetIconSize));
				...
				))
			{
				...
				if(Editor._guiContent_EC_MakeKey == null)
				{
					Editor._guiContent_EC_MakeKey = apGUIContentWrapper.Make(Editor.ImageSet.Get(apImageSet.PRESET.Controller_MakeRecordKey), "Make Key");
				}
				...
				if (isRecordKey)
				{
					...
					//변경
					if (GUILayout.Button(Editor._guiContent_EC_RemoveKey.Content, apGUILOFactory.I.Width(recordBtnSize), apGUILOFactory.I.Height(recordBtnSize)))
					{
				...
				else
				{
					...
					//변경
					if (GUILayout.Button(Editor._guiContent_EC_MakeKey.Content, apGUILOFactory.I.Width(recordBtnSize), apGUILOFactory.I.Height(recordBtnSize)))
					{
			...
			//변경
			if (GUILayout.Button(Editor._guiContent_EC_SetDefault.Content, apGUILOFactory.I.Width(recordBtnSize), apGUILOFactory.I.Height(recordBtnSize)))
			{
			...
			//변경
			if (GUILayout.Button(Editor._guiContent_EC_EditParameter.Content, apGUILOFactory.I.Width(recordBtnSize), apGUILOFactory.I.Height(recordBtnSize)))
			{
			...
			//이거 삭제 > Editor.GUIStyleWrapper.Label_UpperRight를 이용하자
			...
			if ((lastRect.y - windowScrollY) + guiHeight > -10 && lastRect.y - windowScrollY < windowHeight)
			{
				...
				GUI.Box(new Rect(lastRect.x, lastRect.y, width + 10, guiHeight), apStringFactory.I.None);
				...
				case apControlParam.TYPE.Vector2:
					{
						...
						EditorGUILayout.BeginHorizontal(apGUILOFactory.I.Width(width));
						vec2Next.x = EditorGUILayout.DelayedFloatField(vec2Next.x, apGUILOFactory.I.Width(width / 2 - 5));
						vec2Next.y = EditorGUILayout.DelayedFloatField(vec2Next.y, apGUILOFactory.I.Width(width / 2 - 5));
