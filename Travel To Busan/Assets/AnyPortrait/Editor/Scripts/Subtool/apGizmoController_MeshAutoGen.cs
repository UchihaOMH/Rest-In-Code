...
namespace AnyPortrait
{
	...
	public partial class apGizmoController
	{
		...
		/// </summary>
		/// <returns></returns>
		public apGizmos.GizmoEventSet GetEventSet_MeshAutoGen()
		{
			//Morph는 Vertex / VertexPos 계열 이벤트를 사용하며, Color 처리를 한다.

			apGizmos.GizmoEventSet.I.Clear();
			apGizmos.GizmoEventSet.I.SetEvent_1_Basic(	Select__MeshAutoGen,
			...
			apGizmos.GizmoEventSet.I.SetEvent_4_EtcAndKeyboard(	FirstLink__MeshAutoGen, 
																null, 
																null, 
																null, 
																null);
		...
		//-------------------------------------------------------------------------------------------
		// First Link
		public apGizmos.SelectResult FirstLink__MeshAutoGen()
		{
			...
			if(Editor._meshEditeMode_MakeMesh != apEditor.MESH_EDIT_MODE_MAKEMESH.AutoGenerate)
			{
				return null;
			}
		...
		public void Unselect__MeshAutoGen()
		{
			if(Editor.Select.SelectionType != apSelection.SELECTION_TYPE.Mesh
				|| Editor.Select.Mesh == null
				|| Editor.Select.Mesh.LinkedTextureData == null
