...
namespace AnyPortrait
{
	...
	public class apDialog_MaterialLibrary : EditorWindow
	{
		...
		private string[] _packageNames = new string[] 
		{
			"Advanced Presets",
			"LWRP Unlit Preset",
			"LWRP 2D Lit Preset (Experimental)",
			"VR Presets",
			"KeepAlpha Presets",
			"URP Presets"//<<추가 20.1.24
		};
		...
		public void Init(apEditor editor, apPortrait portrait)
		{
			...
			_editor.Controller.LinkMaterialSets();
		...
		void OnGUI()
		{
			try
			{
				...
				if(GUILayout.Button(new GUIContent(" " + _editor.GetText(TEXT.MakeMaterialPreset), _img_MaterialSet), GUILayout.Width(leftWidth - 10), GUILayout.Height(30)))
				{
					//프리셋을 바로 만들자.
					//비어있는 상태로
					_editor.MaterialLibrary.AddNewPreset(null, false, null);
		...
		private void OnGUI_RightUpper(int width)
		{
			...
			Shader nextShader_Normal_AlphaBlend =		ShaderObjectField("   Alpha Blend", _selectedMaterialSet._shader_Normal_AlphaBlend, width);
			Shader nextShader_Normal_Additive =			ShaderObjectField("   Additive", _selectedMaterialSet._shader_Normal_Additive, width);
			Shader nextShader_Normal_SoftAdditive =		ShaderObjectField("   Soft Additive", _selectedMaterialSet._shader_Normal_SoftAdditive, width);
			Shader nextShader_Normal_Multiplicative =	ShaderObjectField("   Multiplicative", _selectedMaterialSet._shader_Normal_Multiplicative, width);

			GUILayout.Space(5);
			EditorGUILayout.LabelField("  " + _editor.GetText(TEXT.ShaderInfo_ClippedRendering));//Clipped Rendering
			Shader nextShader_Clipped_AlphaBlend =		ShaderObjectField("   Alpha Blend", _selectedMaterialSet._shader_Clipped_AlphaBlend, width);
			Shader nextShader_Clipped_Additive =		ShaderObjectField("   Additive", _selectedMaterialSet._shader_Clipped_Additive, width);
			Shader nextShader_Clipped_SoftAdditive =	ShaderObjectField("   Soft Additive", _selectedMaterialSet._shader_Clipped_SoftAdditive, width);
			Shader nextShader_Clipped_Multiplicative =	ShaderObjectField("   Multiplicative", _selectedMaterialSet._shader_Clipped_Multiplicative, width);

			GUILayout.Space(10);
			EditorGUILayout.LabelField(_editor.GetText(TEXT.ShaderInfo_2_CS_Linear));//"2. Color Space : Linear"
			EditorGUILayout.LabelField("  " + _editor.GetText(TEXT.ShaderInfo_BasicRendering));//"  Basic Rendering"
			Shader nextShader_L_Normal_AlphaBlend =		ShaderObjectField("   Alpha Blend", _selectedMaterialSet._shader_L_Normal_AlphaBlend, width);
			Shader nextShader_L_Normal_Additive =		ShaderObjectField("   Additive", _selectedMaterialSet._shader_L_Normal_Additive, width);
			Shader nextShader_L_Normal_SoftAdditive =	ShaderObjectField("   Soft Additive", _selectedMaterialSet._shader_L_Normal_SoftAdditive, width);
			Shader nextShader_L_Normal_Multiplicative =	ShaderObjectField("   Multiplicative", _selectedMaterialSet._shader_L_Normal_Multiplicative, width);

			GUILayout.Space(5);
			EditorGUILayout.LabelField("  " + _editor.GetText(TEXT.ShaderInfo_ClippedRendering));//Clipped Rendering
			Shader nextShader_L_Clipped_AlphaBlend =		ShaderObjectField("   Alpha Blend", _selectedMaterialSet._shader_L_Clipped_AlphaBlend, width);
			Shader nextShader_L_Clipped_Additive =			ShaderObjectField("   Additive", _selectedMaterialSet._shader_L_Clipped_Additive, width);
			Shader nextShader_L_Clipped_SoftAdditive =		ShaderObjectField("   Soft Additive", _selectedMaterialSet._shader_L_Clipped_SoftAdditive, width);
			Shader nextShader_L_Clipped_Multiplicative =	ShaderObjectField("   Multiplicative", _selectedMaterialSet._shader_L_Clipped_Multiplicative, width);

			GUILayout.Space(10);
			Shader nextShader_AlphaMask = ShaderObjectField(_editor.GetText(TEXT.ShaderInfo_3_AlphaMask), _selectedMaterialSet._shader_AlphaMask, width);//"3. Alpha Mask"
		...
		private void OnSelectMaterialSet_MakeMaterialSet(bool isSuccess, object loadKey, apMaterialSet resultMaterialSet, bool isNoneSelected, object savedObject)
		{
			...
			_editor.Controller.AddMaterialSet((isNoneSelected ? null : resultMaterialSet), true, false);
			Repaint();
		}
