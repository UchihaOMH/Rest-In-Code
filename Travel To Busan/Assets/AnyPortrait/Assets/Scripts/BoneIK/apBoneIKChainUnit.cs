	...
	public class apBoneIKChainUnit
	{
		...
		public void ReadyToSimulate(bool isUseIKMatrix = false)
		{
			...
			if (!isUseIKMatrix)
			{
				...
				isXReverse = _baseBone._worldMatrix._scale.x < 0.0f;
			}
			else
			{
				...
				isXReverse = _baseBone._worldMatrix_IK._scale.x < 0.0f;
			}
			...
			if (isXReverse)
			{
				...
				_angleDir_Lower = defaultAngle180 - _baseBone._IKAngleRange_Upper;
				_angleDir_Upper = defaultAngle180 - _baseBone._IKAngleRange_Lower;
			}
			else
			{
				_angleDir_Preferred = defaultAngle180 + _baseBone._IKAnglePreferred;
			...
			if(isUseIKMatrix)
			{
				...
				_angleWorld_Prev = Vector2Angle(_targetBone._worldMatrix._pos - _baseBone._worldMatrix._pos);
			}
		}
