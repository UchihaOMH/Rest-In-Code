...
namespace AnyPortrait
{
	...
	public class apMesh : MonoBehaviour
	{
		...
		public void RefreshVertexID()
		{
			...
			#region [미사용 코드]
			//if(_vertexIDs.Count == _vertexData.Count)
			//{
			//	return;
			//}
			//_vertexIDs.Clear();
			//for (int iVert = 0; iVert < _vertexData.Count; iVert++)
			//{
			//	_vertexIDs.Add(_vertexData[iVert]._uniqueID);
			//} 
			#endregion
		}
		...
		//	while (true)
		//	{
		//		int newID = UnityEngine.Random.Range(100, 9999999);
		//		if(!_vertexIDs.Contains(newID))
		//		{
		//			_vertexIDs.Add(newID);
		//			return newID;
		//		}
		//		cnt++;
		//		if(cnt > 50)
		//		{
		//			for (int i = 100; i < 99999999; i++)
		//			{
		//				if(!_vertexIDs.Contains(i))
		//				{
		//					_vertexIDs.Add(i);
		//					return i;
		//				}
		...
		public void ReadyToEdit(apPortrait portrait)
		{
			...
			//for (int iPolygon = 0; iPolygon < _polygons.Count; iPolygon++)
			//{
			//	apMeshPolygon polygon = _polygons[iPolygon];

			//	//Vert -> Vertex 연결
			//	for (int iVert = 0; iVert < polygon._verts.Count; iVert++)
			//	{
			//		polygon._verts[iVert] = GetVertex(polygon._verts[iVert]._index);
			//	}

			//	//Edge -> Edge 연결
			//	for (int iEdge = 0; iEdge < polygon._edges.Count; iEdge++)
			//	{
			//		apMeshEdge subEdge = polygon._edges[iEdge];

			//		if(subEdge._vert1 != null && subEdge._vert2 != null)
			//		{
			//			polygon._edges[iEdge] = GetEdge(subEdge._vert1._index, subEdge._vert2._index);
			//		}
			//	}

			//	//Hidden Edge -> 내부의 Vertex 연결
			//	for (int iEdge = 0; iEdge < polygon._hidddenEdges.Count; iEdge++)
			//	{
			//		apMeshEdge hiddenEdge = polygon._hidddenEdges[iEdge];

			//		if (hiddenEdge._vert1 != null && hiddenEdge._vert2 != null)
			//		{
			//			hiddenEdge._vert1 = GetVertex(hiddenEdge._vert1._index);
			//			hiddenEdge._vert2 = GetVertex(hiddenEdge._vert2._index);
			//		}
			//	}
			...
			//		tri._verts[0] = GetVertex(tri._verts[0]._index);
			//		tri._verts[1] = GetVertex(tri._verts[1]._index);
			//		tri._verts[2] = GetVertex(tri._verts[2]._index);
			//	}
			//}
#if UNITY_EDITOR
		...
		// 1. Make Mesh
		//----------------------------------------------
		//public void CheckAndMakeMesh(bool isForce)
		//{
		//	if (!isForce)
		//	{
		...
		//	//if (_mesh == null || _meshFilter == null || _meshRenderer == null)
		//	//{
		//	//	Debug.LogError("RefreshMesh Error : No Mesh or MeshFilter or MeshRenderer");
		//	//	return;
		//	//}

		//	if (_vertexData.Count == 0 || _indexBuffer.Count == 0)
		//	{
		//		Debug.LogError("RefreshMesh Error : No Vertex or No Index");
		//		return;
		//	}

		//	_vertices = new Vector3[_vertexData.Count];
		//	_uvs = new Vector2[_vertexData.Count];

		//	for (int i = 0; i < _vertexData.Count; i++)
		//	{
		//		_vertices[i] = _vertexData[i]._pos;
		//		_uvs[i] = _vertexData[i]._uv;
		//	}

		//	_triangles = new int[_indexBuffer.Count];
		//	for (int i = 0; i < _indexBuffer.Count; i++)
		//	{
		//		_triangles[i] = _indexBuffer[i];
		//	}


		//	//for (int i = 0; i < _vertices.Length; i++)
		//	//{
		//	//	Debug.Log("Pos [" + i + "] : " + _vertices[i]);
		//	//}
		...
		//	Debug.Log("Refresh Mesh Complete");
		//}
		...
		public void AutoLinkEdges()
		{
			...
			{
			//return (int)((a._length * 10000.0f) - (b._length * 10000.0f));
			return b.Score - a.Score;
			//return (int)b._minAngle - (int)a._minAngle;
		});
