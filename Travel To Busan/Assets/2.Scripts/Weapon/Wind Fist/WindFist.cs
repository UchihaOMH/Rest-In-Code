...
public class WindFist : Weapon
{
    ...
    { 
        ...
        {
            _target.BeAttacked(owner, owner.info.damage + weaponInfo.damage, _target.transform.position.x - owner.transform.position.x < 0f ? Vector2.left : Vector2.right, weaponInfo.knockBackPower);
        });
    ...
    public override void RizingAttack(float _additionalDmg)
    {
        ...
        fx.AddOnHitEvent((Entity _target) =>
        {
            _target.BeAttacked(owner, owner.info.damage + weaponInfo.damage, _target.transform.position.x - owner.transform.position.x < 0f ? Vector2.left : Vector2.right, weaponInfo.knockBackPower, 1f);
