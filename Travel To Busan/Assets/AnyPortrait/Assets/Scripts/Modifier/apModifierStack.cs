...
namespace AnyPortrait
{
	...
	public class apModifierStack
	{
		...
		public void SetExclusiveModifierInEditing(apModifierBase targetModifier, apModifierParamSetGroup targetParamSetGroup, bool isColorCalculated)
		{
			...
			if (targetModifier != null && targetModifier.IsTarget_Bone && targetModifier.ModifierType != apModifierBase.MODIFIER_TYPE.Rigging)
			{
		...
		public void LinkModifierStackToRenderUnitCalculateStack(bool isRoot = true, apMeshGroup rootMeshGroup = null)
		{
			...
			//Debug.Log("--------------------------------------------------------------");
			//Debug.Log("LinkModifierStackToRenderUnitCalculateStack [" + _parentMeshGroup._name + " ( Is Root : " + isRoot + ")]");
			//Debug.Log("--------------------------------------------------------------");
				...
				for (int iGroup = 0; iGroup < paramSetGroups.Count; iGroup++)
				{
					...
					for (int iParam = 0; iParam < paramSets.Count; iParam++)
					{
						...
						for (int iModMesh = 0; iModMesh < modMeshes.Count; iModMesh++)
						{
							...
							//{
							//	//체크
							//	//이게 RootMeshGroup에 포함된 RenderUnit이 맞나?
							//	if(!rootMeshGroup._renderUnits_All.Contains(modMesh._renderUnit))
							//	{
							//		Debug.LogError("Mod -> RenderUnit Link 에러 : Root MeshGroup에 포함되지 않는 RenderUnit");
							//	}
							...
							else
							{
								...
								//Debug.LogError("> < New > " + modifier.DisplayName + " / ModMesh [" + iModMesh + "] : " + modMesh._renderUnit.Name);
						...
						if (modBoneRenderUnit != null)
						{
							...
							////ModBone 디버그
							//if(modBones.Count > 0 && !modifier.IsAnimated)
							//{
							//	Debug.Log("--------------------------------------------------------");
							//	Debug.Log("[" + _parentMeshGroup._name + "] : " + modifier.DisplayName + ":" + paramSet.SyncControlParam._keyName + " / " + paramSet.ControlParamValue);
							//	for (int iModBone = 0; iModBone < modBones.Count; iModBone++)
							//	{
							//		apModifiedBone modBone = modBones[iModBone];
							//		Debug.Log("[" + iModBone +"] : " + modBone._bone._name);
							//	}
							//}
							...
							for (int iModBone = 0; iModBone < modBones.Count; iModBone++)
							{
								...
								if (modBone._bone == null || modBone._renderUnit == null)
								{
									...
									//	Debug.LogError(_parentMeshGroup.name + " : ModBone -> Calculate Link Failed <No Bone> : [Bone : " + (modBone._bone != null ? modBone._bone._name : "Null") + ", RenderUnit Exist : " + (modBone._renderUnit != null ? modBone._renderUnit.Name : "Null") + "]");
									//}
									//else if (modBone._renderUnit == null)
									//{
									//	Debug.LogError(_parentMeshGroup.name + " : ModBone -> Calculate Link Failed <No Render Unit> : [Bone : " + (modBone._bone != null ? modBone._bone._name : "Null") + ", RenderUnit Exist : " + (modBone._renderUnit != null ? modBone._renderUnit.Name : "Null") + "]");
									//}

									continue;
								}

								//Debug.Log("[Mod Bone Link : [Root : " + rootMeshGroup._name + " / Cur : " + _parentMeshGroup.name + "] + ["+ modifier.DisplayName + "] >> " + modBone._bone._name);
								...
								else
								{
									...
									//Debug.LogError(" < New > : " + modBone._bone._name);
