	...
	public class apAnimControlParamResult
	{
		...
		public void SetKeyframeResult(apAnimKeyframe keyframe, float weight)
		{
			...
			switch (_targetControlParam._valueType)
			{
				...
				case apControlParam.TYPE.Int:
					...
					_value_Int += (int)(keyframe._conSyncValue_Int * weight);//일단 이게 없어야 함 + Float형으로 바뀌었다.
					break;
		...
		public void AdaptToControlParam()
		{
			...
			switch (_targetControlParam._valueType)
			{
				...
				case apControlParam.TYPE.Int:
					//_targetControlParam._int_Cur = _value_Int;//기존
					_targetControlParam._int_Cur = (int)(_value_Int + 0.5f);//반올림해서 적용
					break;
		...
		public void AdaptToControlParam_Opt(float weight, apAnimPlayUnit.BLEND_METHOD blendMethod)
		{
			...
			switch (_targetControlParam._valueType)
			{
				...
				case apControlParam.TYPE.Int:
					//_targetControlParam.SetCalculated_Int(_value_Int, weight, blendMethod);//기존
					_targetControlParam.SetCalculated_Int((int)(_value_Int + 0.5f), weight, blendMethod);
					break;
