...
namespace AnyPortrait
{
	...
	public class apEditorAnimClipTargetHierarchy
	{
				...
				case CATEGORY.Mesh_Item:
					{
						...
						{
							Editor.Select.SetSubMeshTransformForAnimClipEdit(meshTransform, true, true);
				...
				case CATEGORY.MeshGroup_Item:
					{
						...
						{
							Editor.Select.SetSubMeshGroupTransformForAnimClipEdit(meshGroupTransform, true, true);
				...
				case CATEGORY.Bone_Item:
					{
						...
						if (bone != null)
						{
							Editor.Select.SetBoneForAnimClip(bone, true, true);
		...
		//---------------------------------------------
		//Hierarchy 레이아웃 출력
		public int GUI_RenderHierarchy_Transform(int width, Vector2 scroll, int scrollLayoutHeight, bool isGUIEvent)
		{
			_curUnitPosY = 0;
			int maxLevel = 0;
		...
		public int GUI_RenderHierarchy_Bone(int width, Vector2 scroll, int scrollLayoutHeight, bool isGUIEvent)
		{
			_curUnitPosY = 0;
			int maxLevel = 0;
			//루트 노드는 For문으로 돌리고, 그 이후부터는 재귀 호출
		...
		public int GUI_RenderHierarchy_ControlParam(int width, Vector2 scroll, int scrollLayoutHeight, bool isGUIEvent)
		{
			_curUnitPosY = 0;
			int maxLevel = 0;
			//루트 노드는 For문으로 돌리고, 그 이후부터는 재귀 호출
		...
		{	
			int maxLevel = level;
			unit.GUI_Render(_curUnitPosY, level * 10, width, scroll, scrollLayoutHeight, isGUIEvent, level);
