...
namespace AnyPortrait
{
	...
	public class apEditorHierarchyUnit
	{
		...
		// GUI
		//--------------------------------------------------------------------------
		public void GUI_Render(int posY, int leftWidth, int width, Vector2 scroll, int scrollLayoutHeight, bool isGUIEvent, int level, bool isOrderButtonVisible = false)
		{
			//CheckAndCreateGUIStyle();//<<추가 : GUI Style 생성

			//추가 19.11.22
			//만약 렌더링하지 않아도 된다면 렌더링하지 않고 여백만 주고 넘어가야한다.
			//TODO : 다만, 이 과정에서 Layout - Repaint 비동기 문제로 GUI 에러가 발생한다.
			...
			else
			{
				...
				//bool prevRenderable = _isRenderable;
				_isRenderable = apEditorUtil.IsItemInScroll(posY, HEIGHT, scroll, scrollLayoutHeight);
				//if (_isRenderable != prevRenderable)
				//{
				//	Debug.LogWarning("[" + Event.current.type + "] Render 상태 전환 > " + _isRenderable);
				//}
			}
			...
			if (_isSelected)
			{
				...
				GUI.Box(new Rect(lastRect.x + scroll.x, lastRect.y + HEIGHT, width + 10, HEIGHT), NO_TEXT);
			...
			//이 코드는 삭제:GUIStyle의 색상을 바꾸면 안된다.
			//if(!_isAvailable)
			//{
			...
			switch (_unitType)
			{
				...
				case UNIT_TYPE.Label:
					...
					{
						EditorGUILayout.LabelField(_guiContent_Text.Content, _layoutOption_H_Height);
					}
					break;
