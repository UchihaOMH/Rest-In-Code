...
namespace AnyPortrait
{
	...
	public class apBackup
	{
		...
		private DateTime _async_Timer = new DateTime();
		...
		private DateTime _dateTime_Prev = DateTime.Now;
			...
			try
			{
				if (_isAutoSaveBakeWorking)
				{
					...
					if(isEnd)
					{
						...
						_async_Timer = DateTime.Now;


						_dateTime_Prev = DateTime.Now;
					}
				...
				else
				{
					...
					float deltaTime = (float)DateTime.Now.Subtract(_dateTime_Prev).TotalSeconds;
					if(deltaTime < 20.0f)
					{
						//기본 업데이트 단위는 20초.
						return;
		...
		private bool AutoSaveProcess(bool isAsyncProcess = true)
		{
			try
			{
				...
				float tDelta = (float)(DateTime.Now.Subtract(_async_Timer).TotalSeconds);
						...
						case ASYNC_SAVE_STEP.Step2_FileCreate:
							{
								...
								if (tDelta > 0.5f)
								{
									...
									if (_async_SubProcessIndex >= 1)
									{
										...
										_async_StreamWriter.WriteLine(DateTime.Now.Year.ToString() + DateTime.Now.Month.ToString() + DateTime.Now.Day.ToString() + "_" + DateTime.Now.Hour.ToString() + "_" + DateTime.Now.Minute.ToString());
			...
			for (int i = 0; i < fieldsTotal.Count; i++)
			{
				...
				if (attrs != null && attrs.Length != 0)
				{
					...
					for (int iAttr = 0; iAttr < attrs.Length; iAttr++)
					{
						...
						else if(attrs[iAttr].GetType().Equals(typeof(NonSerializedAttribute)))
						{
							isBackupTargetSkip = true;
						}
					}
		...
		private List<FieldInfo> GetAllFields(object targetObj)
		{
			...
			if (fieldCategory == apBackupUnit.FIELD_CATEGORY.Instance)
			{
				...
				while (true)
				{
					...
					Type baseType = curObjType.BaseType;
							...
							case apBackupUnit.FIELD_CATEGORY.UnityMonobehaviour:
								{
									...
									Type monoType = Type.GetType(curUnit._typeName_Partial);
							...
							case apBackupUnit.FIELD_CATEGORY.UnityObject:
								{
									...
									object instanceObj = Activator.CreateInstance(Type.GetType(curUnit._typeName_Partial));
									if (fi != null)
									{
							...
							case apBackupUnit.FIELD_CATEGORY.Instance:
								{
									...
									Type unitType = Type.GetType(curUnit._typeName_Partial);
									if (unitType == null)
									{
									...
									object instanceObj = Activator.CreateInstance(unitType);
							...
							case apBackupUnit.FIELD_CATEGORY.Array:
								{
									...
									try
									{
										...
										else
										{
											...
											Type arrayType = Type.GetType(curUnit._typeName_Partial);
											Type elementType = arrayType.GetElementType();
											listObject = Array.CreateInstance(elementType, nItem) as IList;
										}
