	...
	public class apSelection
	{
		...
		public void SetAnimClip(apAnimClip animClip)
		{
			...
			if (_animClip != animClip)
			{
				...
				if (_animClip._targetMeshGroup != null)
				{
					...
					//_animClip._targetMeshGroup.LinkBoneListToChildMeshGroupsAndRenderUnits();
					_animClip._targetMeshGroup.RefreshModifierLink();
					//_animClip._targetMeshGroup._modifierStack.ActiveAllModifierFromExclusiveEditing();//<<cnrk
		...
		private void Draw_Overall(int width, int height)
		{
			...
			if (Editor._rootUnitEditMode == apEditor.ROOTUNIT_EDIT_MODE.Setting)
			{
				...
				GUILayout.Space(20);
				//MainMesh에서 해제
				if (GUILayout.Button(Editor.GetUIWord(UIWORD.UnregistRootUnit), apGUILOFactory.I.Width(width), apGUILOFactory.I.Height(20)))//"Unregist Root Unit"
				{
...
//#endif
			...
			_scroll_Timeline.y = GUI.VerticalScrollbar(	new Rect(	lastRect.x + leftTabWidth + 4 + timelineWidth - 15, 
																	lastRect.y, 
																	15, 
		...
		private void DrawEditor_Right2_MeshGroup_Setting(int width, int height)
		{
			...
			if (isSelectedObjectRender)
			{
				...
				if (isMeshTransformDetailRendererable || isMeshGroupTransformDetailRendererable)
				{
					...
					_guiContent_Right2MeshGroup_DetachObject.AppendText(_guiContent_Right2MeshGroup_ObjectProp_Type.Content.text, false);
					_guiContent_Right2MeshGroup_DetachObject.AppendText(apStringFactory.I.Bracket_2_R, true);
					

					//Detach 버튼
					if (GUILayout.Button(_guiContent_Right2MeshGroup_DetachObject.Content))//"Detach [" + strType + "]"
					{
...
#region [미사용 코드]
		...
		private void DrawEditor_Right2_MeshGroup_Modifier(int width, int height)
		{
			...
			EditorGUILayout.BeginVertical(apGUILOFactory.I.Width(width - (50 + 10)));
			GUILayout.Space(5);
			EditorGUILayout.LabelField(Modifier.DisplayName, apGUILOFactory.I.Width(width - (50 + 10)));
			...
			if ((int)(Modifier.CalculatedValueType & apCalculatedResultParam.CALCULATED_VALUE_TYPE.Color) != 0)
			{
													...
													Modifier._isExtraPropertyEnabled, true, width, 20))
				{
					...
					Modifier._isExtraPropertyEnabled = !Modifier._isExtraPropertyEnabled;
					_meshGroup.RefreshModifierLink();//<<이거 다시 연결해줘야 한다.
					Editor.RefreshControllerAndHierarchy(false);
				}
				GUILayout.Space(10);
			}
			...
			EditorGUILayout.BeginHorizontal();
			if (GUILayout.Button(Editor.GetUIWord(UIWORD.LayerUp), apGUILOFactory.I.Width(width / 2 - 5)))//"Layer Up"
			{
				Editor.Controller.LayerChange(Modifier, true);
			}
			if (GUILayout.Button(Editor.GetUIWord(UIWORD.LayerDown), apGUILOFactory.I.Width(width / 2 - 5)))//"Layer Down"
			{
...
#region [미사용 코드] v1.1.8 이전의 UI
			...
			EditorGUILayout.BeginHorizontal(apGUILOFactory.I.Width(width), apGUILOFactory.I.Height(200));
			GUILayout.Space(5);
			...
			GUI.Box(new Rect(lastRect.x + 5, lastRect.y, width, 200), apStringFactory.I.None);
			GUI.backgroundColor = prevColor;


			//Weight 리스트를 출력하자
			EditorGUILayout.BeginVertical(apGUILOFactory.I.Width(width), apGUILOFactory.I.Height(200));
			_scrollBottom_Status = EditorGUILayout.BeginScrollView(_scrollBottom_Status, false, true);
			GUILayout.Space(2);
			int scrollWidth = width - (30);
			EditorGUILayout.BeginVertical(apGUILOFactory.I.Width(scrollWidth), apGUILOFactory.I.Height(200));
			GUILayout.Space(3);
		...
		private void DrawModifierPropertyGUI_Physics(int width, int height)
		{
			...
			EditorGUILayout.BeginVertical(apGUILOFactory.I.Width(valueWidth), apGUILOFactory.I.Height(labelHeight));
		...
		private void DrawEditor_Right2_Animation(int width, int height)
		{
			...
			//변경
			EditorGUILayout.LabelField(curIconGUIContent.Content, apGUILOFactory.I.Width(50), apGUILOFactory.I.Height(50));
