...
namespace AnyPortrait
{
	...
	public class apAnimPlayQueue
	{
		...
		private void PushAllNoActiveRequests()
		{
			...
			for (int i = 0; i < _requests_Live.Count; i++)
			{
				if(_requests_Live[i].IsCancellable)
				{

					//Debug.LogError("[" + i + "] " + _requests_Live[i].RequestType + " / " + _requests_Live[i].Status + " >> Remove");
					targetRequests.Add(_requests_Live[i]);
				}
				//else
				//{
				//	Debug.Log("[" + i + "] " + _requests_Live[i].RequestType + " / " + _requests_Live[i].Status);
				//}
			}
