...
namespace Puppet2D
{
	public class Puppet2D_Editor : EditorWindow
	{
		...
		//static bool EditBlendShapeMode = false;
		//static float BlendshapeFalloff = 1f;
		...
		public static List<Puppet2D_VertexDeform> selectedVertexDeforms = new List<Puppet2D_VertexDeform>();
		public static List<int> workingVertexDeformsIndexes = new List<int>();
		//public static Puppet2D_BlendShape WorkingBlendShape;

		static public string _puppet2DPath;

		static public bool HasGuides = false;

		public static float VoxelScale = 100f;
		public static bool KeepVoxels = false;
		static public int BlurIter = 20;
		...
		void OnGUI()
		{
			...
			if (showLayerSizePanel)
			{
				...
				*/
				...
				GUI.DrawTexture(new Rect(25, 80, 32, 32), ControlTexture, ScaleMode.StretchToFill, true, 10.0F);

				EditorGUI.BeginChangeCheck();
				ControlSize = EditorGUI.Slider(new Rect(80, 90, 150, 20), ControlSize, 0F, .9999F);
				if (EditorGUI.EndChangeCheck())
				{
					ChangeControlSize();
				...
				EditorGUI.BeginChangeCheck();
				_controlSortingIndex = EditorGUI.Popup(new Rect(80, 120, 150, 30), _controlSortingIndex, sortingLayers);
				if (EditorGUI.EndChangeCheck())
				{
			...
			if (currentGUIChoice == GUIChoice.RigginSetup)
			{
				...
				if (GUI.Button(new Rect(80, 210 + offsetControls, 150, 30), "Create Blend Shape"))
				{
					CreateBlendShape.MakeBlendShape();
					//WorkingBlendShape = Selection.activeGameObject.GetComponent<Puppet2D_BlendShape>();
					//selectedVertexDeforms.Clear();
					//EditBlendShapeMode = true;
				}

				if (GUI.Button(new Rect(80, 240 + offsetControls, 150, 30), "Clear Blend Shapes"))
				{
					CreateBlendShape.ClearBlendShapes();

				}
				/*if (EditBlendShapeMode)
				{
					GUI.backgroundColor = Color.green;

					if (GUI.Button(new Rect(80, 270 + offsetControls, 150, 30), "Finish Blend Shape"))
					{
						EditBlendShapeMode = false;
						WorkingBlendShape = null;
					}
					EditorGUI.BeginChangeCheck();
					BlendshapeFalloff = EditorGUI.Slider(new Rect(80, 310 + offsetControls, 150, 20), BlendshapeFalloff, 0F, 5f);
					if (EditorGUI.EndChangeCheck())
					{
				...
				{
					GUI.backgroundColor = bgColor;
					if (GUI.Button(new Rect(80, 270 + offsetControls, 150, 30), "Edit Blend Shape"))
					{
						WorkingBlendShape = Selection.activeGameObject.GetComponent<Puppet2D_BlendShape>();
						selectedVertexDeforms.Clear();

						if (WorkingBlendShape!=null)
							EditBlendShapeMode = true;
				...
				if (GUI.Button(new Rect(80, 260 + offsetControls, 150, 30), "Finish Edit Skin Weights"))
				{
					if (SkinWeightsPaint)
					{
						if (currentSelection)
						{
							...
							Puppet2D_Bone[] Bones = Transform.FindObjectsOfType<Puppet2D_Bone>();
		...
		void OnSceneGUI(SceneView sceneView)
		{
			...
			/*if (EditBlendShapeMode)
			{
				...
				{
					//EditBlendShapeMode = false;
					WorkingBlendShape.verts = WorkingBlendShape.mesh.vertices;
					if (WorkingBlendShape != null)
					{
						for (int i = 0; i < WorkingBlendShape.Deforms.Length; i++)
						{
							if (WorkingBlendShape.Deforms[i] != null)
								WorkingBlendShape.verts[i] = WorkingBlendShape.Deforms[i].transform.position;
						}
					}
				}
				if (selectedVertexDeforms.Count > 0)
				{
					//Undo.RecordObjects(WorkingBlendShape.Deforms, "Record Deforms");
					Vector3 newOffset = (selectedVertexDeforms[0].transform.position - selectedVertexDeforms[0].IntialPos);
					foreach (int deformIndex in workingVertexDeformsIndexes)
					{
						if (!WorkingBlendShape.Deforms[deformIndex].Selected)
						{
							Undo.RecordObject(WorkingBlendShape.Deforms[deformIndex].transform, "deform");						

							WorkingBlendShape.verts[deformIndex] = WorkingBlendShape.Deforms[deformIndex].IntialPos + WorkingBlendShape.Deforms[deformIndex].CurrentBlend * newOffset;

							WorkingBlendShape.Deforms[deformIndex].transform.position = WorkingBlendShape.verts[deformIndex];
						}
					}
					foreach (Puppet2D_VertexDeform selVD in selectedVertexDeforms)
					{
						WorkingBlendShape.verts[selVD.CurrentIndex] = selVD.transform.position;
					}
				}
			}*/
			//if (WorkingBlendShape != null)
		...
		void OnSelectionChange()
		{
			...
			//else if (WorkingBlendShape != null)
			//	EditBlendShapeMode = true;
		}
		/*void UpdateBlendShapeControls()
		{
			WorkingBlendShape.Falloff = BlendshapeFalloff;
			foreach (int vdIndex in workingVertexDeformsIndexes)
			{
				Undo.RecordObject(WorkingBlendShape.Deforms[vdIndex], "deform");
				WorkingBlendShape.Deforms[vdIndex].ResetValues();


			}
			foreach (Puppet2D_VertexDeform selVD in selectedVertexDeforms)
			{
				Undo.RecordObject(selVD, "deform");
				selVD.ResetValues();
			...
			selectedVertexDeforms.Clear();
			workingVertexDeformsIndexes.Clear();
			...
			foreach (GameObject go in gos)
			{
				...
				if (vd != null)
				{
					selectedVertexDeforms.Add(vd);

					int[] vdIndexes = vd.GetFalloffVertexDeformIndexes();
					workingVertexDeformsIndexes.AddRange(vdIndexes);

					foreach (int vdIndex in workingVertexDeformsIndexes)
					{
						WorkingBlendShape.Deforms[vdIndex].Selected = false;
						WorkingBlendShape.Deforms[vdIndex].Editted = false;

					}
					foreach (Puppet2D_VertexDeform selVD in selectedVertexDeforms)
					{
						selVD.Selected = false;
						selVD.Editted = false;
		...
		static void RefreshMeshColors()
		{
			...
			if (!BlackAndWhiteWeights)
			{
				...
				for (int j = 0; j < Bones.Length; j++)
				{
					...
					int boneIndex = smr.bones.ToList().IndexOf(Bones[j].transform);
			...
			if (c && c.GetComponent<Puppet2D_Bone>())
			{
				if (BlackAndWhiteWeights)
				{
					Puppet2D_Bone[] Bones = Transform.FindObjectsOfType<Puppet2D_Bone>();
					foreach (Puppet2D_Bone Bone in Bones)
					{
						if (Bone.transform == c.transform)
						{
							Bone.Color = new Color(1, .5f, 0);
						}						
						else
						{
							Bone.Color = Color.white;
