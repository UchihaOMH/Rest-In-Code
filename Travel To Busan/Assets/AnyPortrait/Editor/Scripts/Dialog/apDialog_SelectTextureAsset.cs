	...
	public class apDialog_SelectTextureAsset : EditorWindow
	{
		...
		public void Init(apEditor editor, apTextureData targetTextureData, object loadKey, FUNC_SELECT_TEXTUREASSET_RESULT funcResult)
		{
			...
			try
			{
				...
				_iLoadingFolder = 0;
				_nLoadingFolders = childDirInfos.Length;
				//한번에 몇개씩 업데이트를 해야할까
				//최대 20번의 스탭을 두자
		...
		private void DrawGUI_Loaded(int width, int height)
		{
			...
			int mainListHeight = height - (90 + 10 + 60);
			...
			for (int iRow = 0; iRow < _texture2Ds_Searched.Count; iRow += nImagePerRow)
			{
				...
				{
					int iTex = iRow + iCol;
					if (iTex >= _texture2Ds_Searched.Count)
					{
						break;
					}
					...
					DrawTextureUnit(_texture2Ds_Searched[iTex], imageUnitWidth, imageUnitHeight);
		...
		private void DrawTextureUnit(TextureAssetData textureAssetData, int width, int height)
		{
			int btnSize = width;
			...
			{
				//int imgWidth = textureAssetData._texture.width;
				//if (imgWidth <= 0) { imgWidth = 1; }

				//int imgHeight = textureAssetData._texture.height;
				...
				if (_curSelectedTexture2D == textureAssetData)
				{
				...
				if (GUILayout.Button(textureAssetData._texture, _guiStyle_Img, GUILayout.Width(width), GUILayout.Height(btnSize)))
				{
