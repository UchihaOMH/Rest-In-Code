		...
		public int SortAndRefreshKeyframes()
		{
			...
			if (_activeKeyframes.Count > 0)
			{
				...
				if (isLoop)
				{
					...
					//	{
					//		//1) 둘다 각각 Start/End에 위치한다. => F/L 지정 후 더미 없음
					//		_firstKeyFrame = keyA;
					//		_lastKeyFrame = keyB;
					//	}
					//	else if(keyA._frameIndex == startFrame && keyB._frameIndex != endFrame)
					//	{
					//		//2) Start에는 위치하지만 End는 비었다. => F 지정후 F의 더미가 L이 된다.
					//		_firstKeyFrame = keyA;
					...
					//	else if(keyA._frameIndex != startFrame && keyB._frameIndex == endFrame)
					//	{
					//		//3) End에는 위치하지만 Start는 비었다. => L 지정후 L의 더미가 F가 된다.
					//		_lastKeyFrame = keyB;
					...
					//	else//if(keyA._frameIndex != startFrame && keyB._frameIndex != endFrame)
					//	{
					//		//4) Start와 End에 모두 비었다. => 양쪽 모두 F/L이 되며, 서로 상대에 대해 Dummy가 된다.
					//		_firstKeyFrame = keyA;
					//		_lastKeyFrame = keyB;
					//		keyA.SetDummy(true, keyA._frameIndex + lengthFrame);
					//		keyB.SetDummy(false, keyB._frameIndex - lengthFrame);
				...
				//{
				//	//1. Loop 일때
				//	//1) 마지막 프레임이 EndFrame의 값을 가진다 -> EndFrame이 LastFrame이다.
				//	//2) 마지막 프레임이 EndFrame의 값을 가지지 않는다 -> StartFrame이 LastFrame으로 반복(더미) 한다.
