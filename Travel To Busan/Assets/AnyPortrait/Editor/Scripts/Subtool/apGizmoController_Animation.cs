...
namespace AnyPortrait
{
	...
	public partial class apGizmoController
	{
		...
		public apGizmos.SelectResult Select__Animation(Vector2 mousePosGL, Vector2 mousePosW, int btnIndex, apGizmos.SELECT_TYPE selectType)
		{
			...
			{
				//Vertex를 선택했다면
				Editor.Select.SetBoneForAnimClip(null, false, false);//자동 타임라인 레이어 선택은 생략
			}
			else
			{
				//Vertex를 선택하지 않았다면
			...
			if (!isSelectionLock)
			{
				if (isBoneTarget && !isVertexSelected)
				{
						...
						selectType == apGizmos.SELECT_TYPE.Add)
					{
						...
						if (resultBone != null)
						{
							Editor.Select.SetSubMeshTransformForAnimClipEdit(null, false, false);
							Editor.Select.SetSubMeshGroupTransformForAnimClipEdit(null, false, false);
							Editor.Select.SetBoneForAnimClip(resultBone, true, true);
				...
				if (!isBoneSelected)
				{
					...
					Editor.Select.SetBoneForAnimClip(null, true, false);
				}
			}
			...
			if (!isSelectionLock)
			{
				if (isTransformTarget && !isVertexSelected && !isBoneSelected)
				{
						...
						selectType == apGizmos.SELECT_TYPE.Add)
					{
						...
						{
							//걍 선택을 하자
							//Editor.Select.SetBone(null);//이거 삭제 19.11.22
							Editor.Select.SetBoneForAnimClip(null, false, false);
							Editor.Select.SetSubMeshGroupTransformForAnimClipEdit(null, false, false);
							Editor.Select.SetSubMeshTransformForAnimClipEdit(selectedMeshTransform, true, true);//<<수정 : 바뀐 여부 상관없이 그냥 선택한다.
				...
				{
					Editor.Select.SetSubMeshTransformForAnimClipEdit(null, true, false);
				}
			}
		...
		public void Unselect__Animation()
		{
			...
			else
			{
				...
				if ((int)(Editor.Select.AnimTimeline._linkedModifier.CalculatedValueType & apCalculatedResultParam.CALCULATED_VALUE_TYPE.VertexPos) != 0)
				{
					...
					//Editor.Select.SetBone(null);//이거 말고
					Editor.Select.SetBoneForAnimClip(null, false, false);//이거 쓰자
		...
		public void Move__Animation_Transform(Vector2 curMouseGL, Vector2 curMousePosW, Vector2 deltaMoveW, int btnIndex, bool isFirstMove)
		{
			...
			apAnimTimelineLayer animTimelineLayer = Editor.Select.AnimTimelineLayer;
			apModifierBase linkedModifier = Editor.Select.AnimTimeline._linkedModifier;
			apAnimKeyframe workKeyframe = Editor.Select.AnimWorkKeyframe;
		...
		private bool OnHotKeyEvent__Animation_Transform__Keyboard_Move(Vector2 deltaMoveW, bool isFirstMove)
		{
			...
			else
			{
				...
				if (bone._isIKTail)
				{
					...
					Vector2 bonePosW = bone._worldMatrix._pos + deltaMoveW;//<<키보드 전용 코드 
		...
		public apGizmos.TransformParam PivotReturn__Animation_Transform()
		{
			...
			if (isTransformPivot)
			{
				...
				int transformDepth = targetModMesh._renderUnit.GetDepth();
		...
		private apGizmos.TransformParam GetPivotReturnWhenAutoKey_Transform()
		{
			...
			if (Editor.Select.AnimClip == null 
				|| Editor.Select.AnimClip._targetMeshGroup == null 
				|| Editor.Select.AnimTimeline == null
				|| Editor.Select.AnimTimelineLayer == null)
			{
				return null;
			}
			...
			//Debug.Log("GetPivotReturnWhenAutoKey_Transform");
			apModifierBase linkedModifier = Editor.Select.AnimTimeline._linkedModifier;
			if(linkedModifier == null)
			{
			...
			//Debug.Log("ModType - " + Editor.Select.AnimTimelineLayer._linkModType);

			if (Editor.Select.AnimTimelineLayer._linkModType == apAnimTimelineLayer.LINK_MOD_TYPE.Bone
				&& Editor.Select.AnimTimelineLayer._linkedBone != null
				&& Editor.Select.AnimTimelineLayer._linkedBone == Editor.Select.Bone)
			{
				//1. Bone
				if (Editor._boneGUIRenderMode == apEditor.BONE_RENDER_MODE.None)
				{
			...
			else if (Editor.Select.AnimTimelineLayer._linkModType == apAnimTimelineLayer.LINK_MOD_TYPE.MeshTransform
				&& Editor.Select.AnimTimelineLayer._linkedMeshTransform != null
				&& Editor.Select.SubMeshTransformOnAnimClip == Editor.Select.AnimTimelineLayer._linkedMeshTransform)
			{
				//2. Mesh Transform
				apTransform_Mesh meshTransform = Editor.Select.SubMeshTransformOnAnimClip;
			...
			else if (Editor.Select.AnimTimelineLayer._linkModType == apAnimTimelineLayer.LINK_MOD_TYPE.MeshGroupTransform
				&& Editor.Select.AnimTimelineLayer._linkedMeshGroupTransform != null
				&& Editor.Select.SubMeshGroupTransformOnAnimClip == Editor.Select.AnimTimelineLayer._linkedMeshGroupTransform)
			{
				//3. MeshGroup Transform
				apTransform_MeshGroup meshGroupTransform = Editor.Select.SubMeshGroupTransformOnAnimClip;
