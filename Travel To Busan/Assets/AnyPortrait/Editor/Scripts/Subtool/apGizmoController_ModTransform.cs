	...
	public partial class apGizmoController
	{
		...
		public void TransformChanged_Extra__Modifier_Transform()
		{
			...
			if (Editor.Select.ExEditingMode == apSelection.EX_EDIT.None 
				|| Editor.Select.ExValue_ModMesh == null 
		...
		////TODO : Bone IK Controller가 추가되었다.
		//public void TransformChanged_BoneIKController__Modifier_Transform(float boneMixWeight)
		//{
		//	if (Editor.Select.MeshGroup == null ||
		//		Editor.Select.Modifier == null)
		//	{
		...
		//	//ModBone을 선택했는지 여부 확인후 처리한다.
		//	bool isTargetBone = Editor.Select.Bone != null
		//							&& Editor.Select.ModBoneOfMod != null
		//							&& Editor.Select.Modifier.IsTarget_Bone
		//							&& Editor.Select.ModBoneOfMod._bone == Editor.Select.Bone;

		//	//Mod Bone을 선택하지 않았으면 패스
		//	if (!isTargetBone)
		//	{
		//		return;
		//	}

		//	if(Editor.Select.Bone._IKController._controllerType == apBoneIKController.CONTROLLER_TYPE.None)
		//	{
		//		//Bone IK를 지원하지 않는다면
		//		return;
		//	}
		...
		//	//Undo
		//	object targetObj = Editor.Select.ModBoneOfMod;
		//	apEditorUtil.SetRecord_Modifier(apUndoGroupData.ACTION.Modifier_Gizmo_BoneIKTransform, Editor, Editor.Select.Modifier, targetObj, false);

		//	Editor.Select.ModBoneOfMod._boneIKController_MixWeight = boneMixWeight;//<<값 자체는 0~1의 값을 보장한다.
		//	Editor.Select.MeshGroup.RefreshForce();
		//}
		...
		public apGizmos.TransformParam PivotReturn__Modifier_Transform()
		{
			...
			if (isTransformPivot)
			{
				...
				int transformDepth = Editor.Select.ExValue_ModMesh._renderUnit.GetDepth();
